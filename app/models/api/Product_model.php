...
class Product_model extends CI_Model
{
    ...
    public function get_product_by_category($category_id){
          ...
          WHERE sma_products.category_id = '$category_id';");
        if (!is_null($products)) {
    ...
    public function get_product_by_subcategory($category_id,$sub_category_id){
          ...
          FROM sma_products
          INNER JOIN sma_units ON sma_products.unit=sma_units.id
          WHERE sma_products.category_id = '$category_id' AND sma_products.subcategory_id ='$sub_category_id'");
        if (!is_null($products)) {
            return ( ['products' => $products->result()]);
        } else {
            return array(
    ...
    public function get_product_varients($product_id){
       $product_variants = $this->db->query("SELECT * FROM sma_product_variants WHERE product_id = '$product_id';");
